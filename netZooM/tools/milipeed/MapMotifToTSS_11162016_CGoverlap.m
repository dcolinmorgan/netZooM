chipdir='AllCGoverlap_r10_p4/'; % directory with input bed files
outdir='PromoMapRefSeq_100kb_mQTL/'; % directory to print mapped files (make sure this is already created)
...
fid=fopen('./refseq_hg19_11162016', 'r');
...
for(cnt=mindix:1:maxidx)
	...
	A=textscan(fid, '%s%f%f%s', 'delimiter', '\t');
	fclose(fid);
	filter=ismember(A{1}, uchr); % filter which peaks to map
	if(sum(~filter))
		for(c=1:length(A))
			A{c}=A{c}(filter);
		end
	...
	for(peakcnt=1:length(peaksummit))
		if(filter(peakcnt))
			fprintf(fid, '%s\t%s\t%s\t%.1f\n', [A{1}{peakcnt}, ':', num2str(A{2}(peakcnt)), '-', num2str(A{3}(peakcnt))], A{4}{peakcnt}, LocGene{peakcnt}, DistVals(peakcnt));
		end
	end
	fclose(fid);
	timelapse=toc;
